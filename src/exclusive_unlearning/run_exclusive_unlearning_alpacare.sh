...
# e.g. `DATA_DIR=/mnt/data OUT_ROOT=/mnt/out bash run_exclusive_unlearning_alpacare.sh`
DATA_DIR="${DATA_DIR:-data}"
OUT_ROOT="${OUT_ROOT:-out}"
OUT_DIR="${OUT_ROOT}/medical_sft/alpacare/${MODEL_NAME}"
...
python src/exclusive_unlearning/run_exclusive_unlearning.py \
  ...
  --logging_steps 50 \
  --report_to wandb \
  --wandb_project "medical_sft_${MODEL_NAME}_entropymax" \
  --forget_sample_file "${DATA_DIR}/forget_samples/${MODEL_DATA}/sampled_texts.jsonl" \
  --run_name "${MODEL_NAME}-medical_sft" \
  --eval_text_file "${DATA_DIR}/forget_samples/${MODEL_DATA}/sampled_texts_for_evaluate_loss.jsonl" \
  --forget_lambda 0.4 \
  --seq_len 256 \
  --retain_train_data_file "${DATA_DIR}/MedInstruct-train.json" \
  --retain_eval_data_file "${DATA_DIR}/MedInstruct-test.jsonl"